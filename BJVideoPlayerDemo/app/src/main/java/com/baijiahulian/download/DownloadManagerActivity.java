...
public class DownloadManagerActivity extends AppCompatActivity implements View.OnClickListener, ExecutorWithListener.OnAllTaskEndListener {
    ...
    public void onRemoveSpecific(View view) {
        String taskKeys = etTaskKey.getText().toString().trim();
        String[] keys = taskKeys.split(",");
        for (String key : keys) {
            downloadManager.removeTask(key);
        }
    }
                ...
                switch (downloadInfo.getState()) {
                    ...
                    case DownloadManager.ERROR:
                        ...
                        downloadManager.addDownloadVideoTask(downloadInfo.getFileName(), downloadInfo.getVideoId(), token, downloadInfo.getVideoType(),
                                downloadInfo.getEncryptType(), downloadInfo.getExtraInfo(), new VideoDownloadManager.OnVideoInfoGetListener() {
                                    @Override
                    ...
                    case DownloadManager.WAITING:
                        downloadManager.resumeTaskNow(downloadInfo.getTaskKey());
                        break;
